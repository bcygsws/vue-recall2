...
- 具名插槽：就是子组件中 slot 占位符中声明了，name="slotName"属性，然后和父组件中 template 标签中 v-slot:slotName。这种
  比较好理解，子组件提供占位符（内容填充位置）。父组件提供填充内容

#### 匿名插槽（或者叫默认插槽）

- 匿名插槽：在子组件中只提供 slot 占位符，不提供 name 属性（或者 name="default"）,就会自动为其填充父组件中那个 template 标签中
  没有声明 v-slot 属性的 html 片段

#### 作用域插槽

- 作用域插槽：是一种带数据的插槽，或者叫带参数的插槽。子组件提供参数，父组件内容只填充到子组件占位符中。用来将数据渲染到子组件插槽中或展示在插槽中

#### 注意事项

- 父组件中提供填充内容模板，子组件中没有提供占位符（子组件中一对 slot 标签），那么模板中的内容不会填充到组件中
- 如果子组件中没有匿名插槽，父组件中 template 模板中，没有提供对应的 v-slot:slotName,那么模板中的内容也不会填充到组建中
- 如果子组件中有多个匿名插槽(或者子组件 slot 标签中 name="default"，按照匿名插槽的方式处理)，那么父组件中的没有定义 v-slot:slotName 属性的模板内容，都会填充到匿名插槽中
